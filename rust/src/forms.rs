...
fn form_cond(env: &Rc<RefCell<Env>>, exprs: &[ValueRef]) -> Result<ValueRef, Error> {
    for expr in exprs.iter() {
        ...
        let test_expr = clause.first().ok_or(Error::new("cond: Malformed condition".into()))?;
...
fn make_func_value(env: &Rc<RefCell<Env>>, name: &str, params_value: &ValueRef, body: &[ValueRef]) -> Result<ValueRef, Error> {
    let mut params = vec![];
...
fn form_define(env: &Rc<RefCell<Env>>, exprs: &[ValueRef]) -> Result<ValueRef, Error> {
    let target = exprs.get(0).ok_or(Error::new("define: Malformed args".into()))?;
...
fn form_lambda(env: &Rc<RefCell<Env>>, exprs: &[ValueRef]) -> Result<ValueRef, Error> {
    let params_value = exprs.get(0).ok_or(Error::new("lambda: Malformed args".into()))?;
...
fn form_let(env: &Rc<RefCell<Env>>, exprs: &[ValueRef]) -> Result<ValueRef, Error> {
    ...
    let bindings_value =
        exprs.get(0).ok_or(Error::new("let: Malformed args".into()))?;
...
fn form_set(env: &Rc<RefCell<Env>>, exprs: &[ValueRef]) -> Result<ValueRef, Error> {
    ...
    let value = Env::evaluate(env, &exprs[1])?;
    let var =
        Env::lookup_ref(env, &name).ok_or(Error::new("set!: Name not found".into()))?;
    (*var).borrow_mut().value = value.clone();
    Ok(ValueRef::new(Value::Undef))
}
