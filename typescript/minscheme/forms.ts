...
function makeLambdaFunc(env: Environment, name: string, rawParamsValue: Value, body: Value[]): Value {
...
function lambda(env: Environment, rawParamsValue: Value, ...body: Value[]): Value {
...
function ifs(env: Environment, rawTest: Value, rawThen: Value, rawElse: Value): Value {
...
interface letEnvFunc {
  (envs: { curEnv: Environment, origEnv: Environment}): { nextEnv: Environment, evalEnv: Environment }
}

function letCommon(name: string, env: Environment, rawArgs: Value[], letEnvFunc: letEnvFunc): Value {
...
function letPlain(env: Environment, ...rawArgs: Value[]): Value {
  return letCommon('let', env, rawArgs, function({ curEnv, origEnv }: { curEnv: Environment, origEnv: Environment}) {
...
function letRec(env: Environment, ...rawArgs: Value[]): Value {
  return letCommon('letrec', env, rawArgs, function({ curEnv, origEnv }: { curEnv: Environment, origEnv: Environment}) {
